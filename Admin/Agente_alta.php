<?php
  ...
  if (!isset($_SESSION['usuario'])) {
    ...
    $nombre = $_SESSION['misesion'];
        ...
        $sql = "SELECT * FROM usuarios WHERE (Rol='Agente' || Rol='Comercialización') AND Estatus='Activo'";
        $result = $conn->query($sql);

        $sql2 = "SELECT * FROM zonas ";
...
<body >
    ...
    <!-- Inicio header -->
       ...
       <section class="padre">
          <section class="logo">
              <section class="eslogan">Crear agente de venta </section>
              <section class="img_logo"></section>
          </section>
          <form id="form_registro" name="form_registro" action="../PHP/Agregar_agente.php" method="post">
              <section class="datos">
                ...
                <article class="select_servicio">
                    <select class="selector-sericio" id="selectAgente" name="selectAgente">
                        <?php
                          while($row = $result->fetch_assoc()){
                            echo '
                            <option value="'.$row['id_Usuario'].'">'.$row['Nombre'].' '.$row['Apellido_Paterno'].'</option>
                            ';
                          }                       
                        ?> 
                    </select>      
                  ...
                  <article class="select_servicio">
                      ...
                      <select class="selector-sericio" id="selectZona" name="selectZona">
                        <option value=""> ----Zonas----</option>
                        <?php
                          while($row = $result2->fetch_assoc()){
                            echo '
                            <option value="'.$row['id_Zona'].'">'.$row['Zona']. $row['Apellido_Paterno'].'</option>
                            ';
                          }                       
                        ?>  
                          </select>
